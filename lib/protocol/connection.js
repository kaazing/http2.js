...
Connection.prototype._updateStreamLimit = function _updateStreamLimit(newStreamLimit) {
  ...
  if (this._streamLimit !== Infinity) {
    this._streamSlotsFree += newStreamLimit - this._streamLimit; 
  } else {
    this._streamSlotsFree = newStreamLimit;
  }
  
  this._streamLimit = newStreamLimit;


  this._log.trace({ free: this._streamSlotsFree, streamLimit: this._streamLimit },
                  'Update stream limit.');
...
Connection.prototype._streamStateChange = function _removeStream(stream, state) {
  if (state === 'CLOSED') {
    if (!stream._closedByUs) {
      ...
      var removeStreamTimer = setTimeout(this._removeStream.bind(self, stream), 30000); // TODO: make timeout configurable
    }
  }
};
...
Connection.prototype._initializeSettingsManagement = function _initializeSettingsManagement(settings) {
  ...
  var initialSettings = {};
  assign(initialSettings, this.constructor.defaultSettings); // Merge default settings
  assign(initialSettings, settings); // Apply user settings

  // * Sending the initial settings.
  this._log.debug({ settings: initialSettings },
                  'Sending the first SETTINGS frame as part of the connection header.');
  
  this.set(initialSettings);

  // * Set Listeners to forward SETTINGS changes to the `_receiveSettings` methods
  ...
  // Emit SETTINGS Listeners with initial settings
  this._emitReceiveSettings(initialSettings);
};

Connection.prototype._emitReceiveSettings = function _initializeSettingsManagement(settings) {

  // Apply Settings
  for (var name in settings) {
    this._log.trace({ name: name,  value: settings[name]}, 'Initialize settings ' + name +' update.');
...
Connection.prototype._receiveSettings = function _receiveSettings(frame) {
  ...
  // * If it's a setting change request, then send an ACK and change the appropriate settings
    ...
    this._log.trace({ frame: frame }, 'Receive settings.');

    // Do not push ACK if closed
    if (!this._closed) {
      this.push({
        type: 'SETTINGS',
        flags: { ACK: true },
    ...
    // Emit SETTINGS Listeners with new frame settings
    this._emitReceiveSettings(frame.settings);
  }
};
...
Connection.prototype._initializeFlowControl = function _initializeFlowControl() {
  ...
  this._initialStreamWindowSize = this._initialStreamWindowSize || INITIAL_STREAM_WINDOW_SIZE;
