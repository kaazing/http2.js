...
Stream.prototype._onHeaders = function _onHeaders(frame) {
  if (frame.priority !== undefined) {   // DPW TODO, can see headers to see if should do encoding (streams are unidirectional)
...
Stream.prototype._receive = function _receive(frame, ready) {
  ...
  if (!this._ended && (frame.type === 'DATA')) {
    ...
    {
      var data = frame.data;
      var unzipped = pako.ungzip(data);
      var moreNeeded = this.push(Buffer.from(unzipped));
    }
    else
    {
        var moreNeeded = this.push(frame.data);   // DPW This is where we force data up to API, can decode here
    }
    if (!moreNeeded) {
