...
// By default DeprecatedHeaders trigger only warning.
IncomingMessage.prototype.allowDeprecatedHeaders = true; 
...
IncomingMessage.prototype._validateHeaders = function _validateHeaders(headers) {
  ...
  for (var i = 0; i < deprecatedHeaders.length; i++) {
    ...
    if (key in headers || (key === 'te' && headers[key] !== 'trailers')) {
      if (this.allowDeprecatedHeaders) {
...
// By default DeprecatedHeaders trigger error.
OutgoingMessage.prototype.allowDeprecatedHeaders = false; 
...
OutgoingMessage.prototype.setHeader = function setHeader(name, value) {
  if (this.headersSent) {
    ...
    if (deprecatedHeaders.indexOf(name) !== -1) {
      if (this.allowDeprecatedHeaders === false) {
