...
function Server(options) {
  ...
  if ((options.key && options.cert) || options.pfx) {
    this._log.trace('Creating HTTP/2 server over TLS');
    this._mode = 'tls';
  ...
  // HTTP2 over plain TCP (Perhaps this should be deprecated??)
    ...
    this._log.trace('Creating HTTP/2 server over plain TCP');
    this._mode = 'plain';
...
Server.prototype._fallback = function _fallback(socket) {
  ...
  this._log.trace({ client: socket.remoteAddress + ':' + socket.remotePort,
...
Server.prototype.listen = function listen(port, hostname) {
  this._log.trace({ on: ((typeof hostname === 'string') ? (hostname + ':' + port) : port) },
...
Server.prototype.close = function close(callback) {
  this._log.trace('Closing server');
...
IncomingRequest.prototype._onHeaders = function _onHeaders(headers) {
  ...
  this._log.trace({ method: this.method, scheme: this.scheme, host: this.host,
...
OutgoingResponse.prototype.writeHead = function writeHead(statusCode, reasonPhrase, headers) {
  ...
  this._log.trace({ status: statusCode, headers: this._headers }, 'Sending server response');
...
OutgoingResponse.prototype.push = function push(options) {
  ...
  this._log.trace({ method: promise[':method'], scheme: promise[':scheme'],
  ...
  // * HTTP/2 over TLS negotiated using NPN or ALPN, or fallback to HTTPS1
    ...
    var negotiated = function () {
      ...
      if (started) {
        ...
        if (endpoint) {
          self._log.trace({ e: endpoint, server: options.host + ':' + options.port },
...
OutgoingRequest.prototype._retryAfter = function _retryAfter(retryAfterDelay) {
  ...
  var retryAfterDelayMs = Math.max(retryAfterDelay, 
  ...
  // DEBUG:
  //console.log(retryAfterDelay, retryAfterDelayMs);
...
OutgoingRequest.prototype._start = function _start(stream, options, endpoint) {
  ...
  this._log.trace({ scheme: headers[':scheme'], method: headers[':method'],
...
OutgoingRequest.prototype._onPromise = function _onPromise(stream, headers) {
  this._log.trace({ push_stream: stream.id }, 'Receiving push promise');
...
IncomingResponse.prototype._onHeaders = function _onHeaders(headers) {
  ...
  if (
    ...
    // * Signaling that the headers arrived.
    self._log.trace({ status: self.statusCode, headers: self.headers}, 'Incoming response with retry-after delay:' + retryAfterDelay);
  ...
  // * Signaling that the headers arrived.
  self._log.trace({ status: self.statusCode, headers: self.headers}, 'Incoming response');
  self.emit('ready');
};
