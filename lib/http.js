...
function Server(options) {
  ...
  // HTTP2 over generic socket server
  // DPW TODO create generic stream
  // else if (options.transport) {
  //     this._log.info('Creating HTTP/2 server over generic stream');
  //     this._mode = 'plain';
  //     this._server = options.transport(start);
  // }

  // HTTP2 over plain WebSocket
  else if (options.transport){
      var http = require('http');
      var websocket = require('websocket-stream');

      this._log.info('Creating HTTP/2 server over plain WebSocket');
      this._mode = 'plain';

      var httpServer = http.createServer();
      options.server = httpServer;
      this._server = websocket.createServer(options, start);
      this._server.listen = function(options, cb){
